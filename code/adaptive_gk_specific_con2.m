function [G,GK] = adaptive_gk_specific_con2( AA, BB, psi, dpsi,tol, I )
% [G,GK] = adaptive_gk_specific_con2( AA, BB, psi, dpsi,tol, I )
% This function computes the controllability Gramian for the wave equation
% numerical example through an adaptive Gauss-Kronrod scheme. 
% INPUTS 
% - AA: is the A matrix of the LTI system
% - BB: input matrix B of the LTI system
...
while err_est > tol
    ...
    for i = 1:size(I,1) 
        ...
        Rcon = @(z) ((psi(1i.*z)*speye(n)-AA)\BB) * sqrt(dpsi(1i.*z)); % half part of the Gramian
        a = I(i,1);
        b = I(i,2);
        middle_matrix = zeros(n,15*m); 
        ...
        for j = 1:15
            ...
            middle_matrix(:,((j-1)*m+1):(j*m)) = full(Rcon(t./(1-t.^2))); 
            w(j) = ((t.^2+1)./(1-t.^2).^2); 
        end
